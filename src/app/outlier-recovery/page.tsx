...
export default function OutlierRecoveryPage() {
  ...
  return (
    <div className="wrap">
      ...
      <style jsx>{`
        .wrap { max-width: 920px; margin: 0 auto; padding: 0 20px 80px; color: #e7ecf5; }
        .toolbar { display: flex; align-items: center; justify-content: space-between; padding: 20px 0; }
        .back { display: inline-flex; align-items: center; gap: 6px; color: #94a3b8; text-decoration: none; font-size: 13px; font-weight: 600; }
        .back:hover { color: #fff; }
        .brand { display: inline-flex; align-items: center; gap: 8px; font-weight: 800; font-size: 15px; color: #fff; }
        .brand img { width: 24px; height: 24px; border-radius: 6px; object-fit: cover; }

        .hero { text-align: center; padding: 28px 0 8px; }
        .badge { display: inline-flex; align-items: center; gap: 6px; font-size: 12px; font-weight: 700; letter-spacing: .04em; text-transform: uppercase; color: #6ee7b7; background: rgba(16,185,129,.1); border: 1px solid rgba(16,185,129,.25); padding: 6px 12px; border-radius: 9999px; }
        .hero h1 { font-size: clamp(30px, 5.5vw, 52px); line-height: 1.05; font-weight: 900; letter-spacing: -.02em; margin: 18px 0 0; color: #fff; }
        .grad { background: linear-gradient(90deg, #22d3ee, #22c55e); -webkit-background-clip: text; background-clip: text; color: transparent; }
        .sub { max-width: 640px; margin: 16px auto 0; color: #aab4c5; font-size: 16px; line-height: 1.7; }

        .timer { display: inline-flex; align-items: center; gap: 8px; margin-top: 24px; background: rgba(220,38,38,.12); border: 1px solid rgba(220,38,38,.3); color: #fca5a5; padding: 10px 16px; border-radius: 9999px; font-size: 14px; font-weight: 600; }
        .timer b { color: #fff; font-variant-numeric: tabular-nums; font-size: 15px; }
        .timer.off { background: rgba(100,116,139,.12); border-color: rgba(100,116,139,.3); color: #94a3b8; }

        .block { margin-top: 56px; }
        .h2 { font-size: 22px; font-weight: 800; color: #fff; text-align: center; margin: 0 0 24px; }

        .grid4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; }
        .mini { background: #0f1629; border: 1px solid #1e293b; border-radius: 14px; padding: 18px 16px; display: flex; flex-direction: column; gap: 6px; color: #22d3ee; }
        .mini b { color: #fff; font-size: 14px; }
        .mini span { color: #8a96a8; font-size: 12.5px; }

        .pricing { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; }
        .card { position: relative; background: #0f1629; border: 1px solid #1e293b; border-radius: 18px; padding: 28px 24px; }
        .card.featured { border-color: rgba(34,197,94,.4); box-shadow: 0 0 0 1px rgba(34,197,94,.15), 0 20px 50px -20px rgba(34,197,94,.25); }
        .tag { position: absolute; top: -11px; left: 24px; background: linear-gradient(90deg,#22d3ee,#22c55e); color: #04121b; font-size: 11px; font-weight: 800; padding: 4px 11px; border-radius: 9999px; text-transform: uppercase; letter-spacing: .04em; }
        .card h3 { font-size: 19px; font-weight: 800; color: #fff; margin: 0 0 6px; }
        .cardsub { color: #95a1b3; font-size: 13.5px; line-height: 1.6; margin: 0 0 18px; min-height: 44px; }
        .price { display: flex; align-items: baseline; gap: 10px; }
        .old { color: #64748b; text-decoration: line-through; font-size: 18px; font-weight: 600; }
        .now { color: #fff; font-size: 34px; font-weight: 900; letter-spacing: -.02em; }
        .save { margin-top: 6px; color: #6ee7b7; font-size: 13px; font-weight: 700; }
        .feats { list-style: none; padding: 0; margin: 18px 0 22px; display: flex; flex-direction: column; gap: 10px; }
        .feats li { display: flex; align-items: center; gap: 8px; color: #cdd6e4; font-size: 13.5px; }
        .feats :global(svg) { color: #22c55e; flex-shrink: 0; }

        .cta { display: inline-flex; align-items: center; justify-content: center; gap: 8px; width: 100%; background: linear-gradient(90deg,#22c55e,#16a34a); color: #04121b; font-weight: 800; font-size: 15px; padding: 13px 18px; border-radius: 12px; text-decoration: none; border: none; }
        .cta:hover { filter: brightness(1.05); }
        .cta.ghost { background: transparent; border: 1px solid #2c3a52; color: #fff; }
        .cta.big { width: auto; margin: 28px auto 0; padding: 15px 32px; display: flex; max-width: 320px; }
        .finePrint { display: flex; align-items: center; justify-content: center; gap: 6px; color: #8a96a8; font-size: 12.5px; margin-top: 16px; }

        .steps { display: grid; grid-template-columns: repeat(3,1fr); gap: 16px; }
        .step { background: #0f1629; border: 1px solid #1e293b; border-radius: 14px; padding: 20px 18px; }
        .step .n { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; border-radius: 9999px; background: rgba(34,211,238,.12); color: #22d3ee; font-weight: 800; margin-bottom: 12px; }
        .step b { color: #fff; font-size: 15px; }
        .step p { color: #95a1b3; font-size: 13px; line-height: 1.6; margin: 6px 0 0; }

        .assure { display: flex; gap: 14px; align-items: flex-start; background: rgba(34,197,94,.06); border: 1px solid rgba(34,197,94,.22); border-radius: 16px; padding: 22px; color: #22c55e; }
        .assure b { color: #fff; font-size: 16px; }
        .assure p { color: #aab4c5; font-size: 14px; line-height: 1.65; margin: 6px 0 0; }

        .faq { display: flex; flex-direction: column; gap: 10px; }
        .faq details { background: #0f1629; border: 1px solid #1e293b; border-radius: 12px; padding: 4px 18px; }
        .faq summary { cursor: pointer; color: #fff; font-weight: 700; font-size: 14.5px; padding: 14px 0; list-style: none; }
        .faq summary::-webkit-details-marker { display: none; }
        .faq p { color: #95a1b3; font-size: 13.5px; line-height: 1.65; margin: 0 0 14px; }
        .foot { display: flex; align-items: center; justify-content: center; gap: 6px; color: #64748b; font-size: 12px; margin-top: 26px; }

        @media (max-width: 720px) {
          .grid4, .pricing, .steps { grid-template-columns: 1fr; }
